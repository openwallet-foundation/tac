...
OpenWallet Foundation projects are required to maintain a standard set of files in each repository. This
...
-   [`LICENSE`](https://www.apache.org/licenses/LICENSE-2.0.txt) - https://www.apache.org/licenses/LICENSE-2.0.txt\
    (Unless an exception has been made by the OpenWallet Foundation Governing Board)
...
-   `README` \
    A description of the project that contains information or links to information such as
    -   A reference to the Apache license (required).
    -   The current and important past releases
    -   Documentation for developers and users
-   `MAINTAINERS` \
    A list of all current maintainers with contact info. [A separate document](maintainers-file-content.md)
    covers the specifics.
-   `CONTRIBUTING` \
    Directions on how to contribute code to the project, or a link to a page with that information.
-   `CHANGELOG` \
...
-   Continuous Integration / Continuous Delivery (CICD) configurations \
    Configurations needed to run CICD on OpenWallet Foundation provided systems.
...
-   Apache License Header information in each source code file. \
    For new files added to OpenWallet Foundation repositories they SHOULD include the snippet `SPDX-License-Identifier: Apache-2.0` as part of the header. 
